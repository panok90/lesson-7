...
class Cell:
    ...
    def __add__(self, other):
        object_add = self.number + other.number
        return Cell(object_add)

    def __sub__(self, other):
        object_sub = abs(self.number - other.number)
        if object_sub == 0:
            object_sub = f'Клетка уничтожена!'
        return Cell(object_sub)

    def __mul__(self, other):
        object_mul = self.number * other.number
        return Cell(object_mul)

    def __truediv__(self, other):
        if self.number > other.number:
            object_truediv = self.number // other.number
        else:
            object_truediv = other.number // self.number
        return Cell(object_truediv)

    def __str__(self):
        return f'Количество ячеек в клетке: {self.number}'

    def make_order(self, number_line):
        line = str()
        item = 0
        integer_part = self.number // number_line
        fractional_part = self.number % number_line
        while item < integer_part:
            line = line + ('*' * number_line)
            if (not item == integer_part - 1) or fractional_part:
                line = line + '\n'
            item += 1
        if fractional_part:
            line = line + ('*' * fractional_part)
        print(line)
